...
export type InferenceCandidate = { className: string; crop: string; disease: string; confidence: number }
export type InferencePrediction = { className: string; crop: string; disease: string; confidence: number; candidates?: InferenceCandidate[] }
...
export function normalizeClassName(className: string): { className: string; crop: string; disease: string; confidence: number } | null {
  const [crop, ...diseaseParts] = className.split('___')
  if (!crop || diseaseParts.length === 0) return null
  const rawDisease = diseaseParts.join('___').replace(/_/g, ' ').trim()
  if (!rawDisease) return null
  const cleanCrop = crop.replace(/\(including_sour\)/gi, '').replace(/\(maize\)/gi, '').replace(/_/g, ' ').trim()
  const disease = rawDisease.toLowerCase() === 'healthy' ? 'Healthy' : rawDisease.replace(/\b\w/g, (letter) => letter.toUpperCase())
  return { className, crop: cleanCrop || crop.trim(), disease, confidence: 0 }
}
...
export async function infer(input: InferenceInput): Promise<InferenceResult> {
  ...
  try {
    ...
    const raw = payload.prediction as { class_name?: unknown; confidence?: unknown; candidates?: Array<{ class_name?: unknown; confidence?: unknown }> }
    ...
    if (Array.isArray(raw.candidates)) {
      ...
      for (const item of raw.candidates) {
        if (typeof item?.class_name === 'string' && typeof item?.confidence === 'number') {
          const norm = normalizeClassName(item.class_name)
          if (norm) {
            norm.confidence = item.confidence
            candidates.push(norm)
          }
        }
      }
      return { status: 'completed', prediction: { ...prediction, candidates }, model: payload.model ?? modelInfo(), inferenceTimeMs: typeof payload.inferenceTimeMs === 'number' ? payload.inferenceTimeMs : undefined }
    }
