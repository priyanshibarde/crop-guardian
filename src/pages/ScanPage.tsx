...
type UploadState = 'idle' | 'checking' | 'not-leaf' | 'demo-only'
export function ScanPage() {
  ...
  const analyze = async () => {
    if (source.kind === 'uploadedImage') {
      ...
      setUploadState(validation.suitable ? 'demo-only' : 'not-leaf')
      return
    }
    setLoading(true)
  ...
  return <>
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_.8fr]">
      ...
      <div>
        ...
        <button disabled={loading || uploadState === 'checking'} onClick={analyze} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-coral px-5 py-4 font-extrabold text-white disabled:opacity-60">{loading || uploadState === 'checking' ? <><span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"/>{loading ? 'Analyzing leaf health…' : 'Checking photo…'}</> : <><ScanLine size={19}/>{isDemoScan ? t.analyze : 'Check photo'}</>}</button>
      </div>
    </div>
  </>
}
